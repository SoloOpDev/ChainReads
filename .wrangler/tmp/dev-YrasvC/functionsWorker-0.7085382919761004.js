...
// .wrangler/tmp/bundle-IUzfYB/checked-fetch.js
...
// .wrangler/tmp/pages-dMWUlo/functionsWorker-0.7085382919761004.mjs
...
async function onRequestPost2(context) {
  ...
  try {
    ...
    console.log(`\u{1F4E5} Received ${posts.length} posts for update`);
...
async function onRequestGet2(context) {
  ...
  if (!category || !["trading", "airdrop"].includes(category)) {
  ...
  try {
    ...
    const posts = await storage.getTelegramPosts(category, limit);
...
__name(fetchCoinDeskRSS, "fetchCoinDeskRSS");
__name2(fetchCoinDeskRSS, "fetchCoinDeskRSS");
...
function extractTag(xml, tagName) {
  ...
  if (match2 && match2[1]) {
    return match2[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/&quot;/g, '"').replace(/&#39;/g, "'").trim();
  }
  return null;
}
__name(extractTag, "extractTag");
__name2(extractTag, "extractTag");
function extractImageFromContent(xml) {
  const patterns = [
    /<media:content[^>]+url="([^"]+)"/i,
  ...
  for (const pattern of patterns) {
    const match2 = xml.match(pattern);
    if (match2 && match2[1]) {
      return match2[1];
    }
  }
...
__name(extractImageFromContent, "extractImageFromContent");
__name2(extractImageFromContent, "extractImageFromContent");
async function onRequestGet4(context) {
  ...
  try {
    ...
    console.log("\u{1F504} Fetching fresh RSS data from CoinDesk...");
    const rssData = await fetchCoinDeskRSS();
    try {
      if (rssData.results && rssData.results.length > 0) {
        console.log("\u{1F4BE} Storing articles in D1 database...");
    ...
    try {
      console.log("\u{1F504} Trying database fallback...");
      ...
      if (articles && articles.length > 0) {
        ...
        return new Response(JSON.stringify(fallbackData2), {
          headers: {
            "Content-Type": "application/json",
            "Cache-Control": "public, max-age=300"
          }
...
async function onRequest(context) {
  try {
    ...
    const articles = guides.slice(0, 50).map((guide, index) => ({
...
__name(getSampleGuides, "getSampleGuides");
__name2(getSampleGuides, "getSampleGuides");
...
var schema_d1_exports = {};
__export(schema_d1_exports, {
  cryptoPanicResponseSchema: /* @__PURE__ */ __name(() => cryptoPanicResponseSchema, "cryptoPanicResponseSchema"),
  insertIpBindingSchema: /* @__PURE__ */ __name(() => insertIpBindingSchema, "insertIpBindingSchema"),
  insertNewsArticleSchema: /* @__PURE__ */ __name(() => insertNewsArticleSchema, "insertNewsArticleSchema"),
  insertPredictionSchema: /* @__PURE__ */ __name(() => insertPredictionSchema, "insertPredictionSchema"),
  insertTelegramPostSchema: /* @__PURE__ */ __name(() => insertTelegramPostSchema, "insertTelegramPostSchema"),
  insertUserClaimSchema: /* @__PURE__ */ __name(() => insertUserClaimSchema, "insertUserClaimSchema"),
  insertUserSchema: /* @__PURE__ */ __name(() => insertUserSchema, "insertUserSchema"),
  ipBindings: /* @__PURE__ */ __name(() => ipBindings, "ipBindings"),
  ipBindingsUniqueIndex: /* @__PURE__ */ __name(() => ipBindingsUniqueIndex, "ipBindingsUniqueIndex"),
  newsArticles: /* @__PURE__ */ __name(() => newsArticles, "newsArticles"),
  predictions: /* @__PURE__ */ __name(() => predictions, "predictions"),
  telegramCategoryIndex: /* @__PURE__ */ __name(() => telegramCategoryIndex, "telegramCategoryIndex"),
  telegramDateIndex: /* @__PURE__ */ __name(() => telegramDateIndex, "telegramDateIndex"),
  telegramPosts: /* @__PURE__ */ __name(() => telegramPosts, "telegramPosts"),
  userClaims: /* @__PURE__ */ __name(() => userClaims, "userClaims"),
  userClaimsUniqueIndex: /* @__PURE__ */ __name(() => userClaimsUniqueIndex, "userClaimsUniqueIndex"),
...
var createInsertSchema = /* @__PURE__ */ __name2((entity, refine) => {
  const columns = getColumns(entity);
  return handleColumns(columns, refine ?? {}, insertConditions);
}, "createInsertSchema");
var urlAlphabet = "useandom-26T198340PX75pxJACKVERYMINDBUSHWOLF_GQZbfghjklqvwyzrict";
var nanoid = /* @__PURE__ */ __name2((size = 21) => {
  let id = "";
...
var newsArticles = sqliteTable("news_articles", {
...
var userClaims = sqliteTable("user_claims", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
  userId: text("user_id").notNull().references(() => users.id),
  articleId: text("article_id").notNull(),
  tokensEarned: integer("tokens_earned").notNull(),
  claimedAt: integer("claimed_at", { mode: "timestamp" }).notNull().$defaultFn(() => /* @__PURE__ */ new Date())
});
var ipBindings = sqliteTable("ip_bindings", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
  ipAddress: text("ip_address").notNull(),
  bindingType: text("binding_type").notNull(),
  walletAddress: text("wallet_address").notNull(),
  boundAt: integer("bound_at", { mode: "timestamp" }).notNull().$defaultFn(() => /* @__PURE__ */ new Date())
});
var predictions = sqliteTable("predictions", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
  walletAddress: text("wallet_address").notNull(),
...
var telegramPosts = sqliteTable("telegram_posts", {
  id: text("id").primaryKey(),
...
var userClaimsUniqueIndex = sql`CREATE UNIQUE INDEX IF NOT EXISTS user_claims_user_article_unique ON user_claims(user_id, article_id)`;
var ipBindingsUniqueIndex = sql`CREATE UNIQUE INDEX IF NOT EXISTS ip_bindings_unique ON ip_bindings(ip_address, binding_type)`;
var telegramCategoryIndex = sql`CREATE INDEX IF NOT EXISTS idx_telegram_category ON telegram_posts(category)`;
var telegramDateIndex = sql`CREATE INDEX IF NOT EXISTS idx_telegram_date ON telegram_posts(date DESC)`;
...
var insertUserClaimSchema = createInsertSchema(userClaims).omit({
  id: true,
  claimedAt: true
});
var insertIpBindingSchema = createInsertSchema(ipBindings).omit({
  id: true,
  boundAt: true
});
...
var insertTelegramPostSchema = createInsertSchema(telegramPosts).omit({
  createdAt: true
});
var cryptoPanicResponseSchema = external_exports.object({
  next: external_exports.string().nullable(),
  previous: external_exports.string().nullable(),
  results: external_exports.array(external_exports.object({
    ...
    original_url: external_exports.string(),
    url: external_exports.string().optional(),
    image: external_exports.string().optional(),
    instruments: external_exports.array(external_exports.object({
      code: external_exports.string(),
      title: external_exports.string(),
      slug: external_exports.string().optional()
    })).optional(),
    votes: external_exports.object({
      negative: external_exports.number().optional(),
      positive: external_exports.number().optional(),
      important: external_exports.number().optional(),
      liked: external_exports.number().optional(),
      disliked: external_exports.number().optional(),
      lol: external_exports.number().optional(),
      toxic: external_exports.number().optional(),
      saved: external_exports.number().optional(),
      comments: external_exports.number().optional()
    }).optional(),
    author: external_exports.string().optional()
  }))
});
function createDb(d1) {
  return drizzle(d1, { schema: schema_d1_exports });
}
...
var D1Storage = class {
  ...
  async updateUserTokens(userId, tokenBalance, dailyClaims) {
    const [user] = await this.db.update(users).set({
      tokenBalance,
      dailyClaims,
      lastClaimDate: /* @__PURE__ */ new Date()
    }).where(eq(users.id, userId)).returning();
    return user;
  }
  async resetDailyClaims(userId) {
    const [user] = await this.db.update(users).set({ dailyClaims: 0 }).where(eq(users.id, userId)).returning();
    return user;
  }
  async getNewsArticles(limit = 20, offset = 0) {
  ...
  async createNewsArticle(article) {
    ...
    try {
      const [inserted] = await this.db.insert(newsArticles).values(articleData).returning();
      return {
        ...inserted,
  ...
  async createNewsArticles(articles) {
    ...
    try {
      const inserted = await this.db.insert(newsArticles).values(articlesData).returning();
  ...
  async getUserClaimForArticle(userId, articleId) {
  ...
  async createUserClaim(claim) {
    const [userClaim] = await this.db.insert(userClaims).values(claim).returning();
    return userClaim;
  }
  async getUserDailyClaims(userId, date) {
  ...
  async clearUserClaims(userId) {
    await this.db.delete(userClaims).where(eq(userClaims.userId, userId));
    console.log(`[STORAGE-D1] Cleared claims for user: ${userId}`);
  }
  async getIpBinding(ipAddress, bindingType) {
  ...
  async createIpBinding(binding) {
    const existing = await this.getIpBinding(binding.ipAddress, binding.bindingType);
    if (existing) {
      const [updated] = await this.db.update(ipBindings).set({
      ...
      const [inserted] = await this.db.insert(ipBindings).values(binding).returning();
      return inserted;
    }
  }
  async getWalletBindings(walletAddress, bindingType) {
  ...
  async clearIpBindingsForWallet(walletAddress) {
    const normalizedWallet = walletAddress.toLowerCase();
    await this.db.delete(ipBindings).where(sql`LOWER(${ipBindings.walletAddress}) = ${normalizedWallet}`);
  }
  async clearAllIpBindings() {
    await this.db.delete(ipBindings);
  }
  async createPrediction(prediction) {
    const [pred] = await this.db.insert(predictions).values(prediction).returning();
    return pred;
  }
  async getUserPredictions(walletAddress, limit = 10) {
  ...
  async updatePredictionStatus(id, status, exitPrice, payout) {
    const [pred] = await this.db.update(predictions).set({
      status,
      ...exitPrice !== void 0 && { exitPrice },
      ...payout !== void 0 && { payout }
    }).where(eq(predictions.id, id)).returning();
    return pred;
  }
  async deletePrediction(id) {
    const result = await this.db.delete(predictions).where(eq(predictions.id, id)).returning();
    return result.length > 0;
  }
  async getTelegramPosts(category, limit = 100) {
    ...
    }).from(telegramPosts).where(eq(telegramPosts.category, category)).orderBy(desc(telegramPosts.date)).limit(limit);
  }
  async upsertTelegramPosts(posts) {
    ...
    try {
      ...
      for (let i = 0; i < posts.length; i += BATCH_SIZE) {
        ...
        for (const post of batch) {
          try {
            ...
            if (existing) {
              await this.db.update(telegramPosts).set({
                text: post.text,
                image: post.image || null,
                imageData: post.imageData || null,
                imageFileId: post.imageFileId || null,
              ...
              await this.db.insert(telegramPosts).values(postData);
            }
            inserted++;
          } catch (error) {
      ...
      const tradingPosts = await this.db.select({ id: telegramPosts.id }).from(telegramPosts).where(eq(telegramPosts.category, "trading")).orderBy(desc(telegramPosts.date)).limit(200);
      if (tradingPosts.length === 200) {
        const keepIds = tradingPosts.map((p) => p.id);
        await this.db.delete(telegramPosts).where(and(
          eq(telegramPosts.category, "trading"),
          sql`${telegramPosts.id} NOT IN (${keepIds.map((id) => `'${id}'`).join(",")})`
        ));
      }
      const airdropPosts = await this.db.select({ id: telegramPosts.id }).from(telegramPosts).where(eq(telegramPosts.category, "airdrop")).orderBy(desc(telegramPosts.date)).limit(200);
...
// .wrangler/tmp/bundle-IUzfYB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IUzfYB/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7085382919761004.js.map
